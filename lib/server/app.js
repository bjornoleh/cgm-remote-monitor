...
function create (env, ctx) {
  ...
  // SvelteKit integration - only handle SvelteKit routes, not everything
  const setupSvelteKit = async () => {
    try {
      const svelteKitModule = await import('../../frontend/build/handler.js');
      console.log('SvelteKit handler loaded successfully');

      // Only handle SvelteKit routes under /app/* to avoid conflicts with existing routes
      app.use('/app', svelteKitModule.handler);
      console.log('SvelteKit handler registered for /app/* routes');

      // Also handle the root SvelteKit route if needed, but only for non-API routes
      app.use((req, res, next) => {
        // Skip SvelteKit for API routes, static files, and existing Nightscout routes
        if (req.path.startsWith('/api') ||
            req.path.startsWith('/bundle') ||
            req.path.startsWith('/translations') ||
            req.path.startsWith('/clock') ||
            req.path.startsWith('/pebble') ||
            req.path.startsWith('/swagger') ||
            req.path.startsWith('/api-docs') ||
            req.path.startsWith('/api3-docs') ||
            req.path === '/robots.txt' ||
            req.path === '/sw.js' ||
            req.path === '/' ||
            req.path === '/admin' ||
        ...
        // Let SvelteKit handle other routes
        svelteKitModule.handler(req, res, next);
      });
      console.log('SvelteKit fallback handler registered for unmatched routes');
  ...
  // Initialize SvelteKit (async, won't block server startup)
  setupSvelteKit();
  return app;
}
