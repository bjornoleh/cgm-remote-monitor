...
function init (ctx) {
  ...
  openaps.getEventTypes = function getEventTypes (sbx) {
    ...
    if (units == 'mmol') {
      reasonconf.push({ name: translate('Hypo TT'), targetTop: 7, targetBottom: 7, duration: 45 });
      reasonconf.push({ name: translate('Eating Soon'), targetTop: 4.4, targetBottom: 4.4, duration: 60 });
      reasonconf.push({ name: translate('Activity'), targetTop: 8, targetBottom: 8, duration: 90 });
    } else {
      reasonconf.push({ name: translate('Hypo TT'), targetTop: 125, targetBottom: 125, duration: 45 });
      reasonconf.push({ name: translate('Eating Soon'), targetTop: 80, targetBottom: 80, duration: 60 });
      reasonconf.push({ name: translate('Activity'), targetTop: 140, targetBottom: 120, duration: 90 });
    }
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    sbx.pluginBase.updatePillText(openaps, {
      ...
      (prop.lastEnacted.reason.split(", ").find(x => x.includes("Target")) != undefined ? ' ' + prop.lastEnacted.reason.split(", ").find(x => x.includes("Target")).replace("Target","T") : "") + 
    //  '   ' + prop.lastEnacted.reason.split(", ").find(x => x.includes("ISF")) +
    //  '   ' + prop.lastEnacted.reason.split(", ").find(x => x.includes("CR")) +
      '   sr:' + round(prop.lastEnacted.sensitivityRatio, 2)
      , label: label
      , info: info
