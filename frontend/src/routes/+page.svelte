<script lang="ts">
	...
	import { getClientState, updateClientState, scaleBG, getDirectionInfo } from '$lib/stores/client-state.svelte.ts';
	import { submitTreatment, getAPISecret, type TreatmentData, type APIResponse } from '$lib/api/treatments.ts';
	...
	let lastSubmissionStatus = $state<{ success: boolean; message: string } | null>(null);

	// Handle treatment submission
	async function handleTreatmentSubmit(treatment: TreatmentData): Promise<void> {
	...
	onMount(() => {
		...
		updateClientState({
			...
			brushExtent: [
				new Date(Date.now() - (data.initialData?.focusHours || 3) * 60 * 60 * 1000),
				new Date()
			]
		});
	});
	...
	$effect(() => {
		if (clientState.latestSGV && clientState.settings) {
			const current = clientState.latestSGV;
			const value = scaleBG(current.sgv || current.mgdl || 0, clientState.settings.units);
...
<svelte:head>
	...
	<meta name="description" content="Nightscout - Open Source Continuous Glucose Monitoring" />
</svelte:head>
...
<div class="min-h-screen bg-background text-foreground">
	...
	{:else}
		<div class="container mx-auto px-4 py-6 space-y-6">
			...
			<header class="space-y-4">
					...
					<div class="flex items-center space-x-2">
						...
						<div class="flex items-center gap-2 text-sm text-muted-foreground">
							...
							<div class="flex items-center gap-1">
								<div class="w-2 h-2 rounded-full {clientState.isConnected ? 'bg-green-500' : 'bg-red-500'}"></div>
								<span class="hidden sm:inline">{clientState.isConnected ? 'Connected' : 'Disconnected'}</span>
							</div>
				...
				{#if lastSubmissionStatus}
					<div class="bg-{lastSubmissionStatus.success ? 'green' : 'red'}-100 border border-{lastSubmissionStatus.success ? 'green' : 'red'}-200 text-{lastSubmissionStatus.success ? 'green' : 'red'}-800 px-4 py-2 rounded-lg">
						<div class="flex items-center justify-between">
							<span class="text-sm">{lastSubmissionStatus.message}</span>
							<button onclick={() => lastSubmissionStatus = null} class="text-{lastSubmissionStatus.success ? 'green' : 'red'}-600 hover:text-{lastSubmissionStatus.success ? 'green' : 'red'}-800">
								✕
							</button>
						</div>
					</div>
			...
			{#if clientState.currentAnnouncement}
				<div class="bg-warning text-warning-foreground p-4 rounded-lg border border-warning">
					<div class="flex items-start justify-between">
						<div>
							<h3 class="font-semibold">{clientState.currentAnnouncement.title}</h3>
							<p class="text-sm mt-1">{clientState.currentAnnouncement.message}</p>
						</div>
						<button 
							onclick={() => updateClientState({ currentAnnouncement: undefined })}
		...
		<div class="fixed bottom-6 right-6 z-50">
			<!-- Action Menu -->
			{#if showActionMenu}
				<div class="absolute bottom-16 right-0 bg-background border border-border rounded-lg shadow-lg p-2 min-w-48 space-y-1 action-menu">
					<button 
						onclick={() => { showBolusCalculator = true; showActionMenu = false; }}
					...
					<button 
						onclick={() => { showCarePortal = true; showActionMenu = false; }}
						class="w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-muted rounded-md transition-colors"
					>
						<span class="text-xl">📝</span>
					...
					<button 
						onclick={() => { showSettings = true; showActionMenu = false; }}
						class="w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-muted rounded-md transition-colors"
					>
					...
					<button 
						onclick={() => { showHelpDialog = true; showActionMenu = false; }}
						class="w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-muted rounded-md transition-colors"
					>
						<span class="text-xl">❓</span>
			...
			<button 
				...
				class="w-14 h-14 bg-primary text-primary-foreground rounded-full hover:bg-primary/90 transition-all duration-200 flex items-center justify-center text-xl fab {showActionMenu ? 'rotate-45' : ''}"
...
{#if showSettings}
	<Settings 
		...
		onSettingsUpdate={(newSettings) => {
			updateClientState({ settings: { ...clientState.settings, ...newSettings } });
		}}
	/>
{/if}
...
{#if showHelpDialog}
	<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
		<div class="bg-background border border-border rounded-lg max-w-md w-full max-h-[80vh] overflow-auto help-dialog">
			<div class="p-6">
				<div class="flex items-center justify-between mb-4">
					<h2 class="text-xl font-semibold">Help & Keyboard Shortcuts</h2>
					<button onclick={() => showHelpDialog = false} class="text-muted-foreground hover:text-foreground text-xl">
						✕
					</button>
				</div>
				...
				<div class="space-y-4 text-sm">
					<div>
						...
						<ul class="space-y-1 text-muted-foreground">
							<li><kbd class="bg-muted px-1 rounded">Ctrl+B</kbd> - Open Bolus Calculator</li>
							<li><kbd class="bg-muted px-1 rounded">Ctrl+C</kbd> - Open Care Portal</li>
							<li><kbd class="bg-muted px-1 rounded">Ctrl+,</kbd> - Open Settings</li>
							<li><kbd class="bg-muted px-1 rounded">Ctrl+R</kbd> - Return to Live View</li>
							<li><kbd class="bg-muted px-1 rounded">F1</kbd> - Show this help</li>
							<li><kbd class="bg-muted px-1 rounded">Esc</kbd> - Close dialogs</li>
						</ul>
					</div>
