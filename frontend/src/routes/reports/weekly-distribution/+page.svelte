<script lang="ts">
  ...
  const distributionDataByDay = $derived(
    reportDetails?.distributionByDay || []
  ); // Renamed for clarity
  ...
  const pieChartData = $derived(
    avgWeeklyTIR && tirColors
      ? [
          {
            name: "Very Low (<54)",
            value: avgWeeklyTIR.severeLow,
            color: tirColors.severeLow,
          },
          {
            name: "Low (54-69)",
            value: avgWeeklyTIR.low,
            color: tirColors.low,
          },
          {
            name: "Target (70-180)",
            value: avgWeeklyTIR.target,
            color: tirColors.target,
          },
          {
            name: "High (181-250)",
            value: avgWeeklyTIR.high,
            color: tirColors.high,
          },
          {
            name: "Very High (>250)",
            value: avgWeeklyTIR.severeHigh,
  ...
  {#if reportDetails}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      <div class="lg:col-span-2 bg-white shadow-lg rounded-lg p-4 md:p-6">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">
          Glucose Distribution by Day of Week
        </h2>
          ...
          <div class="h-96 md:h-[500px]">
            ...
            >
              ...
              <Rect
                y0={(d) => +d.q1}
                y1={(d) => +d.q3}
                class="fill-blue-500 opacity-60"
                widthRatio={0.6}
              />
              <Line
                each={(d) => [
              ...
              >
                {#each xPoints as x, i}
                  <line
                    x1={xPoints[i] -
                      (this.xBandwidth ? this.xBandwidth() : 50) * 0.3}
                    y1={yPoints[i]}
                    x2={xPoints[i] +
                      (this.xBandwidth ? this.xBandwidth() : 50) * 0.3}
                    y2={yPoints[i]}
              ...
              >
                {#each xPoints as x, i}
                  <line
                    x1={xPoints[i] -
                      (this.xBandwidth ? this.xBandwidth() : 50) * 0.15}
                    y1={yPoints[i]}
                    x2={xPoints[i] +
                      (this.xBandwidth ? this.xBandwidth() : 50) * 0.15}
                    y2={yPoints[i]}
      ...
      <div class="lg:col-span-1">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Overall Time In Range for the week
            </CardDescription>
          </CardHeader>
          <CardContent class="p-0 flex justify-center items-center">
            ...
            {:else}
              <p class="text-sm text-gray-500 text-center p-4">
                Average Weekly TIR data not available.
              </p>
            {/if}
          </CardContent>
        </Card>
    ...
    {#if distributionDataByDay.length > 0}
      <div class="bg-white shadow-lg rounded-lg p-4 md:p-6">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">
          Detailed Weekly Distribution Data
        </h2>
        <Table>
          <TableCaption class="text-sm text-gray-500 mt-2">
            Min, Q1, Median, Q3, and Max glucose values for each day.
          </TableCaption>
          <TableHeader>
            <TableRow>
