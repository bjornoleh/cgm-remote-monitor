<script lang="ts">  import type { PageData } from "./$types";
  ...
  const summaryStats = $derived.by(() => {
    ...
    const avgMedian = weeklyPercentileData.reduce((sum, week) => sum + week.median, 0) / weeklyPercentileData.length;
    const avgP25 = weeklyPercentileData.reduce((sum, week) => sum + week.p25, 0) / weeklyPercentileData.length;
    const avgP75 = weeklyPercentileData.reduce((sum, week) => sum + week.p75, 0) / weeklyPercentileData.length;
    const avgHigh = weeklyPercentileData.reduce((sum, week) => sum + week.high, 0) / weeklyPercentileData.length;
    const avgLow = weeklyPercentileData.reduce((sum, week) => sum + week.low, 0) / weeklyPercentileData.length;
  ...
  const chartData = $derived(weeklyPercentileData.length > 0 ? weeklyPercentileData : demoData);
</script>

<svelte:head>
  <title>Weekly Overview Report - Nightscout</title>
  <meta name="description" content="Weekly glucose percentile distribution and overview analysis" />
</svelte:head>
...
<div class="container mx-auto px-4 py-6 space-y-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
    ...
    <Card.Root>
      ...
      <Card.Content>
        <div class="text-2xl font-bold text-green-600">{summaryStats.avgTimeInRange}%</div>
    ...
    <Card.Root>
      ...
      <Card.Content>
        <div class="text-2xl font-bold" style="color: {TIR_COLORS_RGB.high}">{summaryStats.avgHigh}</div>
        <p class="text-xs text-muted-foreground">mg/dL</p>
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root>
      ...
      <Card.Content>
        <div class="text-2xl font-bold" style="color: {TIR_COLORS_RGB.low}">{summaryStats.avgLow}</div>
        <p class="text-xs text-muted-foreground">mg/dL</p>
      </Card.Content>
    </Card.Root>
  </div>
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>
        Stacked area chart showing glucose distribution across percentiles over time.
        Each layer represents a different percentile range from 10th to 90th percentile.
      </Card.Description>
    </Card.Header>
    <Card.Content>
      {#if !data.success}
      ...
      {:else}
        <WeeklyGlucosePercentileChart data={chartData} />
      {/if}
    </Card.Content>
  </Card.Root>
      ...
      <Card.Content>
        <div class="overflow-x-auto">
          <table class="w-full border-collapse">
            <thead>
              <tr class="border-b">
                <th class="text-left p-2 font-medium">Week Starting</th>
                <th class="text-right p-2 font-medium">10th %ile</th>
                <th class="text-right p-2 font-medium">25th %ile</th>
                <th class="text-right p-2 font-medium">Median</th>
                <th class="text-right p-2 font-medium">75th %ile</th>
                <th class="text-right p-2 font-medium">90th %ile</th>
                <th class="text-right p-2 font-medium">Range</th>
              </tr>
            </thead>
            <tbody>
              {#each chartData as week}
                <tr class="border-b hover:bg-muted/50">
                  ...
                  <td class="text-right p-2" style="color: {TIR_COLORS_RGB.low}">
                    {Math.round(week.low)}
                  </td>
                  <td class="text-right p-2" style="color: {TIR_COLORS_RGB.low}">
                    {Math.round(week.p25)}
                  </td>
                  <td class="text-right p-2 font-medium" style="color: {TIR_COLORS_RGB.target}">
                    {Math.round(week.median)}
                  </td>
                  <td class="text-right p-2" style="color: {TIR_COLORS_RGB.high}">
                    {Math.round(week.p75)}
                  </td>
                  <td class="text-right p-2" style="color: {TIR_COLORS_RGB.high}">
                    {Math.round(week.high)}
                  </td>
                  <td class="text-right p-2 text-muted-foreground">
                    {Math.round(week.high - week.low)}
                  </td>
                </tr>
              {/each}
            </tbody>
      ...
      <div class="space-y-3">
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              The chart shows your glucose distribution across different percentiles over time.
              The middle band (25th-75th percentile) represents the range where 50% of your readings fall.
            </p>
          </div>
        </div>
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              The green shaded area represents the target glucose range (70-180 mg/dL).
              More overlap with the middle percentiles indicates better glucose control.
            </p>
          </div>
        </div>
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              Week-to-week changes in the percentile bands help identify patterns and
              the effectiveness of treatment adjustments.
            </p>
          </div>
        </div>
      </div>
