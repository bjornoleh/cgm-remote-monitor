<script lang="ts">
  ...
  const pieChartTIRData = $derived(
    mostRecentWeekTIRData && tirColors
      ? [
          {
            name: "Very Low",
          ...
          {
            name: "Low",
            value: mostRecentWeekTIRData.low,
            color: tirColors.low,
          },
          {
            name: "Target",
            value: mostRecentWeekTIRData.target,
            color: tirColors.target,
          },
          {
            name: "High",
            value: mostRecentWeekTIRData.high,
...
<div class="p-4 md:p-6 bg-gray-100 min-h-screen">
  {#if reportDetails}
        ...
        <Card>
          <CardHeader>
            ...
            {#if weeklyDataPoints.length > 0}
              <CardDescription>
                Time In Range for {weeklyDataPoints[weeklyDataPoints.length - 1]
                  .weekLabel}
              </CardDescription>
            {/if}
          </CardHeader>
          <CardContent class="p-0 flex justify-center items-center pt-2">
            ...
            {:else}
              <p class="text-sm text-gray-500 text-center p-4">
                TIR data for pie chart not available.
              </p>
            {/if}
          </CardContent>
        </Card>
      ...
      <div class="bg-white shadow-lg rounded-lg p-4 md:p-6">
        ...
        <Table>
          <TableCaption class="text-sm text-gray-500 mt-2">
            Summary of weekly glucose readings.
          </TableCaption>
          <TableHeader>
            <TableRow>
    ...
    {:else}
      <p class="text-center text-gray-500 py-10">
        No weekly data available for table display.
      </p>
    {/if}
  {:else}
