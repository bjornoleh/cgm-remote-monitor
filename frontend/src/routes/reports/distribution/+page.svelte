<script lang="ts">
  ...
  const histogramChartData = $derived( // Renamed for clarity
  ...
  const pieChartTIRData = $derived(tirPieDataFromServer && tirPieColors ? [
    { name: 'Very Low (<54)', value: tirPieDataFromServer.veryLow, color: tirPieColors.veryLow },
    { name: 'Low (54-69)', value: tirPieDataFromServer.low, color: tirPieColors.low },
    { name: 'Target (70-180)', value: tirPieDataFromServer.target, color: tirPieColors.target },
    { name: 'High (181-250)', value: tirPieDataFromServer.high, color: tirPieColors.high },
    { name: 'Very High (>250)', value: tirPieDataFromServer.veryHigh, color: tirPieColors.veryHigh }
  ...
  {#if reportDetails}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
        {#if summary}
          <Card>
            <CardHeader><CardTitle>Target Range</CardTitle><CardDescription>70-180 mg/dL</CardDescription></CardHeader>
            <CardContent><p class="text-3xl font-bold">{summary.percentTarget.toFixed(1)}%</p></CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Below Target</CardTitle><CardDescription>&lt;70 mg/dL</CardDescription></CardHeader>
            <CardContent><p class="text-3xl font-bold">{(summary.percentVeryLow + summary.percentLow).toFixed(1)}%</p></CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Above Target</CardTitle><CardDescription>&gt;180 mg/dL</CardDescription></CardHeader>
            <CardContent><p class="text-3xl font-bold">{(summary.percentHigh + summary.percentVeryHigh).toFixed(1)}%</p></CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Total Readings</CardTitle></CardHeader>
            <CardContent><p class="text-3xl font-bold">{summary.totalReadings}</p></CardContent>
          </Card>
        {/if}
      </div>
      <div class="lg:col-span-1">
        <Card>
          ...
          <CardContent class="p-0 flex justify-center items-center pt-2"> {/* Added pt-2 */}
    ...
    {#if histogramChartData.length > 0}
      <div class="bg-white shadow-lg rounded-lg p-4 md:p-6 mb-8">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">Distribution of Glucose Readings (%)</h2>
        <div class="h-72 md:h-96">
          <LayerChart data={histogramChartData} x="x" y="y" xDomain={null} yDomain={null} yPadding={0.1}>
            <XAxis dataKey="x" label="Glucose Range (mg/dL)" grid={false}/>
            <YAxis dataKey="y" label="Percentage of Readings (%)" grid={true} ticks={5}/>
    ...
    {#if distributionPoints.length > 0}
      <div class="bg-white shadow-lg rounded-lg p-4 md:p-6">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">Detailed Distribution Data</h2>
        <Table>
          <TableCaption class="text-sm text-gray-500 mt-2">Count and percentage of readings per glucose range.</TableCaption>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {#each distributionPoints as bin (bin.range)}
              <TableRow>
                ...
                <TableCell class="text-right">{bin.percent.toFixed(1)}%</TableCell>
              </TableRow>
            {/each}
          </TableBody>
        </Table>
