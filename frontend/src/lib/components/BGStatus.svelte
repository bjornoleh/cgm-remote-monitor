<script lang="ts">
  ...
  let bgStatusClass = $derived.by(() => {
    if (!currentBG) return "bg-muted";

    const { raw } = currentBG;
    const thresholds = settings.thresholds || {};

    if (raw >= (thresholds.bgHigh || 180))
      return "bg-destructive text-destructive-foreground";
    if (raw <= (thresholds.bgLow || 55))
      return "bg-destructive text-destructive-foreground";
    if (raw > (thresholds.bgTargetTop || 140))
      return "bg-warning text-warning-foreground";
    if (raw < (thresholds.bgTargetBottom || 80))
      return "bg-warning text-warning-foreground";
...
<div
  class="bg-status-container p-4 rounded-lg border {bgStatusClass}"
  class:animate-pulse={alarmInProgress}
>
  <div class="flex flex-col items-center space-y-2">
    ...
    {#if delta && currentBG}
      <div class="text-lg font-medium">
        <span
          class:text-destructive={delta.value > 0}
          class:text-success={delta.value < 0}
        >
          {delta.display}
        </span>
      </div>
    ...
    {#if latestSGV?.noise !== undefined}
      <div class="flex items-center space-x-2 text-xs opacity-60">
        ...
        {#if latestSGV.filtered}
          <span>•</span>
          <span>
            Filtered: {scaleBG(latestSGV.filtered, settings.units || "mg/dl")}
          </span>
        {/if}
      </div>
    {/if}
