...
<script lang="ts">
  ...
  let isRetroMode = $derived.by(() => {
    return useOtherTime && mergedDateTime() < new Date();
  });

  let isFutureMode = $derived.by(() => {
    return useOtherTime && mergedDateTime() > new Date();
  });

  let profileData = $derived.by((): ProfileData => {
  ...
  function gatherBoluscalcData() {
    ...
    if (
      record.targetBGLow === 0 ||
      record.targetBGHigh === 0 ||
      record.isf === 0 ||
      record.ic === 0
    ) {
      ...
      <div class="time-section">
        <div class="time-controls">
          <label>
            <input
              type="radio"
              bind:group={useNowTime}
              value={true}
              onchange={eventTimeTypeChange}
            />
            Use current time
          </label>
          <label>
        ...
        <div class="input-row">
          ...
          <div class="bg-inputs">
            ...
            <label class="checkbox-small">
              <input
                type="checkbox"
                bind:checked={bgFromSensor}
                onchange={updateVisualisations}
              />
              From sensor
            </label>
          </div>
          ...
          <div class="food-list">
            ...
            {#each foods as food, index}
              <div class="food-item">
                <button
                  class="delete-food"
                  onclick={() => deleteFoodRecord(index)}
                >
                  ×
                </button>
                <span class="food-name">{food.name}</span>
                <span class="food-details">
                  {(food.portion * food.portions).toFixed(1)}
                  {food.unit}
        ...
        <div class="result-grid">
          {#if useBG && record.bg > 0}
            <div class="result-item">
              ...
              <span class="detail">
                Target: {record.targetBGLow}-{record.targetBGHigh}
          ...
          {#if useIOB && record.iob !== 0}
            <div class="result-item">
              <label>IOB:</label>
              <span class="value">
                {record.iob > 0 ? "-" : ""}{Math.abs(record.iob).toFixed(2)} U
              </span>
            </div>
          {/if}
