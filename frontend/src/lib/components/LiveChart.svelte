<script lang="ts">
  import { Chart, Svg, Spline, Points, RectClipPath } from "layerchart";
  ...
  function getBGColor(value: number) {
    ...
    if (value > chartConfig.bgTargetTop.value)
      return chartConfig.bgTargetTop.color;
    if (value < chartConfig.bgTargetBottom.value)
      return chartConfig.bgTargetTop.color;
...
<ChartContainer config={chartConfig} class="h-[400px] w-full">
  ...
  >
    <Svg>
      <RectClipPath>
        ...
        <defs>
          <linearGradient id="bgZones" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop
              offset="0%"
              style="stop-color:{chartConfig.bgHigh.color};stop-opacity:0.1"
            />
            <stop
              offset="{((yDomain[1] - chartConfig.bgHigh.value) /
                (yDomain[1] - yDomain[0])) *
                100}%"
              style="stop-color:{chartConfig.bgHigh.color};stop-opacity:0.1"
            />
            <stop
              offset="{((yDomain[1] - chartConfig.bgTargetTop.value) /
                (yDomain[1] - yDomain[0])) *
                100}%"
              style="stop-color:{chartConfig.bgTargetTop
                .color};stop-opacity:0.1"
            />
            <stop
              offset="{((yDomain[1] - chartConfig.bgTargetBottom.value) /
                (yDomain[1] - yDomain[0])) *
                100}%"
              style="stop-color:{chartConfig.bgTargetBottom
                .color};stop-opacity:0.1"
            />
            <stop
              offset="{((yDomain[1] - chartConfig.bgLow.value) /
                (yDomain[1] - yDomain[0])) *
                100}%"
              style="stop-color:{chartConfig.bgLow.color};stop-opacity:0.1"
            />
            <stop
              offset="100%"
              style="stop-color:{chartConfig.bgLow.color};stop-opacity:0.1"
        ...
        <!-- Main BG line -->
        <Spline stroke="hsl(var(--primary))" strokeWidth={2} />
      ...
      <!-- <Axis
        formatTick={(value) =>
          scaleBG(value, settings.units || "mg/dl").toString()}
      />
      <Axis
    ...
    {#if !inRetroMode}
      <!-- <Brush on:brush={handleBrushChange} selection={brushExtent} /> -->
    {/if}
